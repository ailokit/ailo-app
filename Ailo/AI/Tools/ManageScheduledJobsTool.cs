...
public sealed class ManageScheduledJobsTool(CronJobScheduler scheduler)
{
    ...
    public async Task<string> UpdateScheduledJobAsync(
        ...
        [Description("Optional JSON parameters for the job. Notification jobs use {\"title\":\"...\",\"body\":\"...\",\"subtitle\":\"...\"}; agent jobs use {\"prompt\":\"...\",\"workingDirectory\":\"/absolute/path\"}.")] string? parametersJson = null,
        [Description("Optional enabled state. Set false to pause the job, or true to resume it.")] bool? isEnabled = null,
        CancellationToken cancellationToken = default)
    {
        var job = await scheduler.UpdateAsync(jobId, cronExpression, parametersJson, isEnabled, cancellationToken).ConfigureAwait(false);
